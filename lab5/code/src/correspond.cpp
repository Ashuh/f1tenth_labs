                       ...
                       vector<vector<int> >& jump_table, vector<Correspondence>& c, float prob)
{
  ...
  // input : trans_points : vector of struct points containing the new points transformed to the previous frame using
  // the current estimated transform
  ...
  // input : c: vector of struct correspondences . This is a refernece which needs to be updated in place and return the
  // new correspondences to calculate the transforms.
  // output : c; update the correspondence vector in place which is provided as a reference. you need to find the index
  // of the best and the second best point.
