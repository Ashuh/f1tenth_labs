...
public:
  ...
  void handleLaserScan(const sensor_msgs::LaserScan::ConstPtr& msg)
  {
    ...
    while (count < MAX_ITER && (curr_trans != prev_trans || count == 0))
    {
      ...
      getNaiveCorrespondence(prev_points, transformed_points, points, jump_table, corresponds,
                             A * count * count + MIN_INFO);
...
int main(int argc, char** argv)
{
  ...
  // SUBSCRIBE
  ros::Subscriber sub = n.subscribe(TOPIC_SCAN, 1, &ScanProcessor::handleLaserScan, &processor);
